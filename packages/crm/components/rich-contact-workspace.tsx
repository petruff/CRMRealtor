    ...
    <li className="min-w-0 bg-surface p-4 sm:p-5">
      ...
      {!readOnly && !archived ? (
        <details className="mt-3 rounded-xl bg-surface-2 p-3">
          <summary className="inline-flex min-h-9 cursor-pointer items-center text-xs font-medium text-accent">Edit contact point</summary>
          <form action={saveAction} className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <label className="sk-field"><span className="sk-label">Label</span><input name="label" maxLength={80} required defaultValue={point.label} className="sk-input" /></label>
            <label className="sk-field"><span className="sk-label">{point.type === 'phone' ? 'Phone' : 'Email'}</span><input name="displayValue" maxLength={point.type === 'phone' ? 40 : 254} required defaultValue={point.displayValue} className="sk-input" type={point.type === 'phone' ? 'tel' : 'email'} /></label>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
          ...
          <div className="border-t border-line p-4 sm:p-5">
            ...
            {!archived ? (
              <form action={pointAction} className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <label className="sk-field"><span className="sk-label">Label</span><input name="label" required maxLength={80} className="sk-input" placeholder="Mobile, work, personal" /></label>
                <label className="sk-field"><span className="sk-label">{pointType === 'phone' ? 'Phone number' : 'Email address'}</span><input name="displayValue" required maxLength={pointType === 'phone' ? 40 : 254} type={pointType === 'phone' ? 'tel' : 'email'} className="sk-input" /></label>
        ...
        <details className="sk-form-section" open>
          ...
          <div className="border-t border-line p-4 sm:p-5">
            ...
            {!archived ? <form action={householdAction} className="mt-4 flex min-w-0 flex-col gap-2 sm:flex-row"><input type="hidden" name="contactId" value={contact.id} /><label className="sk-field min-w-0 flex-1"><span className="sk-label">New household</span><input name="name" required maxLength={80} className="sk-input" placeholder="Monroe household" /></label><div className="sm:self-end"><Submit><Plus className="size-4" /> Create</Submit></div></form> : null}
        ...
        <details className="sk-form-section xl:col-span-2">
          ...
          <div className="border-t border-line p-4 sm:p-5">
            ...
            {isOwner && !archived ? <form action={definitionAction} className="mt-5 grid gap-3 rounded-xl border border-line p-4 sm:grid-cols-2"><input type="hidden" name="contactId" value={contact.id} /><label className="sk-field"><span className="sk-label">Field name</span><input name="name" required maxLength={80} className="sk-input" /></label><label className="sk-field"><span className="sk-label">Type</span><select name="type" className="sk-input"><option value="text">Text</option><option value="number">Number</option><option value="date">Date</option><option value="boolean">Yes / no</option><option value="single-select">Single select</option></select></label><label className="sk-field sm:col-span-2"><span className="sk-label">Options (single select only)</span><input name="options" className="sk-input" placeholder="Option one, option two" /></label><input type="hidden" name="displayOrder" value={activeDefinitions.length} /><div><Submit><Plus className="size-4" /> Create field</Submit></div></form> : null}
