...
export function WorkspaceAssignmentPanel({ contacts, assignments, members, currentMembershipId }: { contacts: readonly Contact[]; assignments: readonly ContactAssignment[]; members: readonly WorkspaceMembership[]; currentMembershipId: string }) {
  return (
    <section className="mt-10" aria-labelledby="assignment-overview-title">
      ...
      {contacts.length ? <ul className="sk-group grid gap-px lg:grid-cols-2">{contacts.map((contact) => <AssignmentRow key={contact.id} contact={contact} assignments={assignments.filter((assignment) => assignment.contactId === contact.id)} members={members} currentMembershipId={currentMembershipId} />)}</ul> : <p className="rounded-2xl bg-surface-2 p-5 text-sm text-muted">No active contacts are available for assignment.</p>}
    </section>
  );
}
