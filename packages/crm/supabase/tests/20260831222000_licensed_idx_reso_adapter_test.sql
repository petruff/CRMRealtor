...
select ok(not has_function_privilege('authenticated','public.claim_listing_sync(uuid,text,text,text,text,timestamp with time zone)','EXECUTE'),'browser members cannot claim provider work');
