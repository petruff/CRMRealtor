...
begin
  if auth.uid() is null or not exists (
    ...
    where member.id=target_actor_membership_id
      ...
      and member.role='owner' and member.status='active' and member.revoked_at is null
  ) then
...
begin
  ...
  if auth.uid() is null or not exists (
    ...
    where member.id=target_actor_membership_id
      ...
      and member.user_id=auth.uid()
      and member.role='owner' and member.status='active' and member.revoked_at is null
  ) then raise exception 'Only the active workspace owner can roll back historical organization.' using errcode='42501'; end if;
