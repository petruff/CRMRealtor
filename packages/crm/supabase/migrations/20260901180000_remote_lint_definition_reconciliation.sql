...
select pg_temp.replace_function_fragment_flexible('public.record_omnix_inbound_response_intelligence(uuid,uuid,bigint,text,text,text,text,text,text,text,text,text,text[],timestamptz)',$pattern$\mtarget_resource\M[[:space:]]+[A-Za-z0-9_.]+%rowtype[[:space:]]*;$pattern$,'');
select pg_temp.replace_function_fragment_flexible('public.record_omnix_inbound_response_intelligence(uuid,uuid,bigint,text,text,text,text,text,text,text,text,text,text[],timestamptz)',$pattern$select[[:space:]]+resource\.\*[[:space:]]+into[[:space:]]+target_resource$pattern$,'perform 1');
