-- Story 3.31: preserve a valid subset of the guided Google workspace grant.
begin;
...
begin
  ...
  if target_transaction.consumed_at is null
     ...
     or not ('openid'=any(canonical_scopes) and 'email'=any(canonical_scopes))
     or (
       target_transaction.requested_scope_bundle<>'workspace-core'
       and exists(select 1 from unnest(target_transaction.requested_scopes) required_scope
         where required_scope<>all(canonical_scopes))
     )
     or (
       target_transaction.requested_scope_bundle='workspace-core'
       and not exists(
         select 1
         from unnest(target_transaction.requested_scopes) requested_scope
         where requested_scope not in ('openid','email')
           and requested_scope=any(canonical_scopes)
       )
     ) then
...
comment on function public.finalize_google_oauth(uuid,uuid,uuid,uuid,text,text,text[],integer,
  jsonb,integer,jsonb,uuid,timestamptz) is
  'Finalizes a Google OAuth transaction with exact owner binding and independently persisted capability grants.';
