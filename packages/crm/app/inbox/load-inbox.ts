...
import { readinessGaps } from '@/lib/domain/florida-readiness';
...
export async function loadInbox(): Promise<LoadedInbox> {
  ...
  const [contacts, replies, proposals, tasks, transactions, milestones] = await Promise.all([
    ...
    optional(context.transactionRepository.list(scope)),
    optional(context.operationalSignalRepository.listMilestones(scope, { limit: 500 })),
  ]);
  ...
  return {
    projection: buildInboxProjection({
      ...
      ...(transactions && milestones ? { readiness: readinessGaps(transactions, milestones, now) } : {}),
