...
SYNAPSE runs automatically via the Codex hook. To inspect what it did:
...
```
.Codex/hooks/synapse-wrapper.cjs        # Layer 1: Hook Entry (spawns the engine hook)
`-- .Codex/hooks/synapse-engine.cjs     #   Hook implementation (stdin -> stdout)
        |
        v imports
...
| File | Purpose |
|------|---------|
| `.Codex/hooks/synapse-wrapper.cjs` | Hook entry point registered in `.Codex/settings.json` |
| `.Codex/hooks/synapse-engine.cjs` | Hook implementation, spawned by the wrapper |
