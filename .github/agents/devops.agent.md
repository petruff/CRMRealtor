...
- `*pro-request-reset` - Trigger the password reset email flow for an AEXOS Pro account
- `*pro-resend-verification` - Resend the AEXOS Pro email verification link
- `*pro-reset-password` - Reset an AEXOS Pro password administratively and validate login
- `*pro-validate-login` - Validate AEXOS Pro login and return auth health signals
- `*pro-verify-status` - Check AEXOS Pro email verification status for an access token
- `*pro-activate` - Call activate-pro directly to validate or restore AEXOS Pro activation
- `*health-check` - Run unified health diagnostic (aexos doctor --json + governance interpretation)
