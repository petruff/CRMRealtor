...
- Any pipeline (epic, multi-wave initiative) accumulating handoffs in `.aexos/handoffs/`
...
3. `.aexos/run-logs/{pipeline-slug}-RUN-LOG.md` (for framework-level pipelines)
...
### Original handoff
Archived: `.aexos/handoffs/_archive/{filename}.yaml` (or deleted if redundant)
```

### Step 4 — Archive or delete originals

Two options:
- **Archive** (safer): move YAMLs to `.aexos/handoffs/_archive/{pipeline-slug}/` — preserves audit trail
