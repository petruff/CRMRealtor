...
wsl bash -c 'cd /mnt/c/.../aexos-core && ~/.local/bin/coderabbit --severity CRITICAL,HIGH --auto-fix'

# Manual review
wsl bash -c 'cd /mnt/c/.../aexos-core && ~/.local/bin/coderabbit -t uncommitted'
...
Full config in `.aexos-core/core-config.yaml` under `coderabbit_integration` section.
