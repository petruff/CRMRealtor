...
```yaml
# .aexos-core/data/archetype-vocabulary.yaml
...
```javascript
// .aexos-core/infrastructure/scripts/output-formatter.js
...
```javascript
// .aexos-core/infrastructure/scripts/validate-output-pattern.js
...
```markdown
<!-- .aexos-core/templates/agent-checklist-template.md -->
...
```yaml
# .aexos-core/workflows/{workflow-name}.yaml
...
2. **Signature Closings** - Agent-specific sign-off
   - Dex: "— Vulcan, sempre construindo 🔨"
   - Quinn: "— Argus, guardião da qualidade 🛡️"
...
**Tasks:**
1. Update dev.md → Vulcan (Builder)
2. Update qa.md → Argus (Guardian)
3. Update po.md → Themis (Balancer)
4. Update pm.md → Janus (Visionary)
5. Update sm.md → Chronos (Flow Master)
6. Update architect.md → Vega (Architect)
7. Update analyst.md → Sirius (Explorer)
8. Update ux-design-expert.md → Iris (Empathizer)
9. Rename db-sage.md → data-engineer.md → Ceres (Engineer)
10. Rename github-devops.md → devops.md → Polaris (Operator)
11. Merge aexos-developer + aexos-orchestrator → aexos-master.md → Zeus (Orchestrator)
