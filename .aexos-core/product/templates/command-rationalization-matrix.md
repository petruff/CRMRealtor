...
|---|---------|----------|-------|-------|-----------|----------|-----------|----------------|
| 1 | example-command | Framework | aexos-master | HIGH (15) | create-doc.md | ✅ KEEP | Core meta-framework capability | N/A |
| 2 | party-mode | Utility | aexos-master | ZERO (0) | N/A | ❌ REMOVE | Novelty feature, unused | Removed in v3.0 |
...
| 5 | create-prd | Docs | aexos-master | MED (7) | create-doc.md | 🔄 DELEGATE | Better suited for @pm | Use @pm *create-prd |
...
grep -r "\*command-name" docs/ .aexos-core/ | wc -l

# For each agent's commands:
for cmd in $(grep -E "^  - [a-z-]+" .aexos-core/agents/AGENT.md | cut -d':' -f1 | sed 's/  - //'); do
  count=$(grep -r "\*$cmd" docs/ .aexos-core/ 2>/dev/null | wc -l)
  echo "$cmd: $count"
done
```
...
```yaml
- create {type} {name}: Create AEXOS component
