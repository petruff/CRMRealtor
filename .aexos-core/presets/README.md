...
AEXOS Presets are pre-configured bundles that provide ready-to-use configurations for specific use cases, project types, or team structures. Presets simplify the initial setup process and ensure consistent configuration across projects.
...
| ------------ | ----------------------- | ------------------ | ----------------- |
| `minimal`    | Bare minimum AEXOS setup | dev                | Quick prototyping |
...
\`\`\`bash
aexos install --preset my-preset
\`\`\`
```
...
aexos install --preset standard

# Install with multiple presets (layered)
aexos install --preset standard --preset frontend

# Install custom preset from directory
aexos install --preset ./presets/my-preset
```
...
cyryx presets show standard

# Compare two presets
cyryx presets diff minimal standard
```
...
cyryx presets export --name my-project-preset --output ./presets/

# Export with description
cyryx presets export --name my-preset --description "Project template" --output ./presets/
```
...
```bash
aexos install --preset standard --preset frontend --preset small-team
```
...
2. **Review before install** - Use `aexos presets show` to understand what you're getting
3. **Export before modifying** - Save your config as a preset before major changes
4. **Keep presets updated** - Check for preset updates when upgrading AEXOS
