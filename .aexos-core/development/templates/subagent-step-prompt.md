...
> Each variable is replaced at runtime by the orchestrator (aexos-master) before spawning the subagent via the Task tool.
...
|----------|--------|-------------|
| `{{AGENT_NAME}}` | Agent file → `agent.name` | Agent's display name (e.g., "Polaris", "Pedro") |
...
|---------|-----------|-----------|-----------|
| `core` | `.aexos-core/development/agents/{agent}.md` | `.aexos-core/development/tasks/{uses}.md` | `.aexos-core/data/{file}` |
...
2. If not found, check `.aexos-core/development/agents/{agent}.md`
...
- The orchestrator (aexos-master) builds the prompt by reading files and replacing variables
