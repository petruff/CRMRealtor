...
The `aexos-delegate` wrapper launches Codex in non-interactive exec mode and sends the prompt through stdin:
...
`full-auto` is retained as an AEXOS abstraction for background executor runs. On current Codex CLI versions, it maps to `workspace-write` plus `approval=never` rather than bypassing the sandbox.
...
- In background mode, `aexos-delegate` returns after a process is spawned and prints the PID.
- In foreground mode, `aexos-delegate` waits for Codex and returns Codex's exit code.
