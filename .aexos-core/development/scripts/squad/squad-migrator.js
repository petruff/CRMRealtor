...
class SquadMigrator {
  ...
  async analyze(squadPath) {
    ...
    try {
      ...
      // Check for missing cyryx.type
      if (!manifest.cyryx?.type) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_AEXOS_TYPE',
          message: 'Missing required field: cyryx.type',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
      ...
      // Check for missing cyryx.minVersion
      if (!manifest.cyryx?.minVersion) {
        analysis.needsMigration = true;
        analysis.issues.push({
          type: 'MISSING_MIN_VERSION',
          message: 'Missing required field: cyryx.minVersion',
          severity: 'error',
        });
        analysis.actions.push({
          type: 'ADD_FIELD',
          path: 'cyryx.minVersion',
          value: '2.1.0',
        });
      }
