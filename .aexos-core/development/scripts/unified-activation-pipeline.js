 ...
 * Can be overridden via core-config.yaml pipeline.timeout_ms or AEXOS_PIPELINE_TIMEOUT env var.
 * @type {number}
 */
...
const ALL_AGENT_IDS = [
  ...
  'devops', 'aexos-master', 'squad-creator',
];
...
class UnifiedActivationPipeline {
  ...
  async _loadCoreConfig() {
    try {
      const configPath = path.join(this.projectRoot, '.aexos-core', 'core-config.yaml');
   ...
   * Priority: AEXOS_PIPELINE_TIMEOUT env > core-config.yaml pipeline.timeout_ms > default
   * @private
  ...
  _resolvePipelineTimeout(coreConfig) {
    ...
    const envTimeout = process.env.AEXOS_PIPELINE_TIMEOUT;
    if (envTimeout) {
  ...
  _resolveLoaderTimeout(tierName, defaultTimeout) {
    const envKey = `AEXOS_LOADER_TIMEOUT_${tierName.replace(/([A-Z])/g, '_$1').toUpperCase()}`;
