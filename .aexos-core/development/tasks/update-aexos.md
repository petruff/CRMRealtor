# Task: Update AEXOS Framework

> **Version:** 4.0.0
> **Created:** 2026-01-29
> **Updated:** 2026-01-31
> **Type:** SYNC (git-native framework synchronization)
> **Agent:** @devops (Polaris) or @cyryx (Zeus)
> **Execution:** Simple bash script (~15 lines)

## Purpose

Git-native sync of AEXOS framework from upstream repository. Uses sparse clone + file comparison for safe review before applying changes. All local customizations preserved automatically by backup/restore.
...
bash .aexos-core/scripts/update-aexos.sh

# Review changes shown by the script, then:
git add .aexos-core && git commit -m "chore: sync AEXOS framework"   # Apply changes
# OR
git checkout -- .aexos-core/                                         # Cancel changes
```
...
1. **Clone upstream** - Sparse shallow clone of CyryxLabs/AEXOS (only `.aexos-core/`)
...
| Path | Reason |
|------|--------|
| `.aexos-core/squads/` | Custom copywriters, data, ralph |
| `.aexos-core/marketing/` | Marketing-specific agents/tasks |
| `source/` | Business context YAML |
| `Knowledge/` | Knowledge bases |
| `.aexos-core/context/` | Compiled contexts |
...
```yaml
task: updateCYRYXFramework
agent: devops
mode: simple
timeout: 60  # 1 minute max

execution:
  script: .aexos-core/scripts/update-aexos.sh

workflow:
  1. If dirty working tree: git add -A && git commit -m "chore: pre-update commit"
  2. bash .aexos-core/scripts/update-aexos.sh
  3. Review changes displayed
  4. git add .aexos-core && git commit -m "chore: sync AEXOS framework"  # to apply
  5. git checkout -- .aexos-core/                                        # to cancel
...
ls -la .aexos-core/squads/  # if exists
