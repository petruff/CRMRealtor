#!/usr/bin/env node
/**
 * Wave 0 — 3-way .aexos-core structural diff (OSS × hub × enterprise).
 *
 ...
 *
 * Usage (from aexos-core root):
 *   node .aexos-core/infrastructure/scripts/framework-3way-diff.js
 *   node .aexos-core/infrastructure/scripts/framework-3way-diff.js --hub ../peer-hub --enterprise ../peer-ent --json
 *   AEXOS_HUB_ROOT=../peer-hub AEXOS_ENTERPRISE_ROOT=../peer-ent npm run diff:framework-3way
 *
...
function parseArgs(argv) {
  ...
  const args = {
    oss: cwd,
    hub: resolveSibling(cwd, ['hub-framework', 'aexos-hub', 'framework-hub']),
    enterprise: resolveSibling(cwd, ['enterprise-framework', 'aexos-enterprise', 'AEXOS-enterprise']),
    json: false,
    out: null,
    quiet: false,
  ...
  if (process.env.AEXOS_HUB_ROOT) args.hub = path.resolve(process.env.AEXOS_HUB_ROOT);
  if (process.env.AEXOS_ENTERPRISE_ROOT) {
    args.enterprise = path.resolve(process.env.AEXOS_ENTERPRISE_ROOT);
  }
    ...
    if (a === '--oss') args.oss = path.resolve(argv[++i]);
      ...
      console.log(
        'Usage: framework-3way-diff.js [--oss DIR] [--hub DIR] [--enterprise DIR] [--json] [--out FILE] [--require-external] [--include-timestamp]\n' +
          'Env: AEXOS_HUB_ROOT, AEXOS_ENTERPRISE_ROOT'
      );
      process.exit(0);
    }
  }
...
function formatReport(result) {
  const lines = [];
  lines.push('# Framework 3-way diff (`.aexos-core`)');
  lines.push('');
...
function main(argv = process.argv.slice(2)) {
  ...
  if (!oss.present) {
    console.error('OSS .aexos-core not found at', args.oss);
    process.exitCode = 2;
    return result;
  }
