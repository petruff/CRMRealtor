...
/**
 * Git Post-Commit Hook - AEXOS ProjectStatusLoader Cache Invalidation
 *
 ...
 *     node .aexos-core/infrastructure/scripts/git-hooks/post-commit.js
 */
 ...
 * looking for .aexos-core directory.
 *
...
function findProjectRoot() {
  ...
  // Walk up looking for .aexos-core
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, '.aexos-core'))) {
      return dir;
    }
...
/**
 * Clear all project-status cache files in the .aexos directory.
 * Handles both standard cache and worktree-specific cache files.
 */
function clearProjectStatusCache() {
  const projectRoot = findProjectRoot();
  const cyryxDir = path.join(projectRoot, '.aexos');

  if (!fs.existsSync(cyryxDir)) {
    return; // No .aexos directory - nothing to clear
  }
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.startsWith('project-status') && file.endsWith('.yaml')) {
        const filePath = path.join(cyryxDir, file);
        try {
