...
// Layer: L2 (.aexos-core/infrastructure/scripts/)
// Purpose:
...
//
// Usage:
//   node .aexos-core/infrastructure/scripts/generate-optimization-report.js [options]
//
// Options:
...
const ANALYTICS_DIR = path.resolve(ROOT, '.aexos', 'analytics');
...
const TOOL_REGISTRY_FILE = path.resolve(ROOT, '.aexos-core', 'data', 'tool-registry.yaml');
...
function compareBaseline(baseline, usageData, registry) {
  ...
  for (const [wfName, wfData] of Object.entries(baselineWorkflows)) {
    ...
    const baselineOverheadPct = baseline.comparison?.cyryxActual?.overheadPercentOfTypicalSession?.[wfName] || 0;
