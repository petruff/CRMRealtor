...
function getDefaultOptions() {
  ...
  return {
    ...
    agentsDir: path.join(projectRoot, '.gemini', 'rules', 'AEXOS', 'agents'),
    commandsDir: path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: path.join(projectRoot, 'packages', 'gemini-aexos-extension'),
    sourceAgentsDir: path.join(projectRoot, '.aexos-core', 'development', 'agents'),
    quiet: false,
    json: false,
  };
}
...
function validateGeminiIntegration(options = {}) {
  ...
  const resolved = {
    ...
    agentsDir: options.agentsDir || path.join(projectRoot, '.gemini', 'rules', 'AEXOS', 'agents'),
    commandsDir: options.commandsDir || path.join(projectRoot, '.gemini', 'commands'),
    extensionDir: options.extensionDir || path.join(projectRoot, 'packages', 'gemini-aexos-extension'),
    sourceAgentsDir: options.sourceAgentsDir || path.join(projectRoot, '.aexos-core', 'development', 'agents'),
  };
  const errors = [];
  const warnings = [];
  ...
  if (!commandFiles.includes('aexos-menu.toml')) {
    errors.push(`Missing Gemini command file: ${path.relative(resolved.projectRoot, path.join(resolved.commandsDir, 'aexos-menu.toml'))}`);
  }
  ...
  const requiredExtensionFiles = [
    'extension.json',
    'README.md',
    path.join('commands', 'aexos-status.js'),
    path.join('commands', 'aexos-agents.js'),
    path.join('commands', 'aexos-validate.js'),
