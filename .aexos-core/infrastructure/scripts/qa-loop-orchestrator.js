...
const CONFIG = {
  ...
  // Workflow definition
  workflowPath: '.aexos-core/development/workflows/qa-loop.yaml',
  ...
  persistenceIndexPath: '.aexos/qa-loops-index.json', // Track all active loops
};
...
class QALoopOrchestrator {
  ...
  _loadMaxIterations() {
    ...
    const configPaths = [
      path.join(this.rootPath, '.aexos/config.yaml'),
      path.join(this.rootPath, '.aexos/config.yml'),
      path.join(this.rootPath, 'cyryx.config.js'),
    ];
...
function printHelp() {
  console.log(`
📊 QA Loop Orchestrator - AEXOS QA Evolution (Story 6.5)
