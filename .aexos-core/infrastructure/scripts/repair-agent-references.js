...
function buildLegacyArtifacts(agents, options) {
  ...
  artifacts.push({
    kind: 'gemini-menu',
    scope: 'project',
    canonicalId: 'aexos-menu',
    legacyId: 'aexos-menu',
    path: path.join(options.geminiCommandsDir, 'aexos-menu.toml'),
  });
...
function validateGeminiCommands(agents, commandsDir) {
  ...
  const menuPath = path.join(commandsDir, 'aexos-menu.toml');
  checked.push(menuPath);
  if (!fs.existsSync(menuPath)) {
    errors.push(`Missing Gemini launcher: ${menuPath}`);
  }

  const legacyMenuPath = path.join(commandsDir, 'aexos-menu.toml');
