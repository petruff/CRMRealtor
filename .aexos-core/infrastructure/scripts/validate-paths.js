...
const GENERATED_SKILL_MARKER = '<!-- CYRYX-CODEX-LOCAL-SKILLS: generated -->';
...
function getDefaultOptions() {
  ...
  return {
    ...
    requiredFiles: [
      ...
      path.join(projectRoot, '.aexos-core', 'product', 'templates', 'ide-rules', 'codex-rules.md'),
    ],
    quiet: false,
    json: false,
  };
...
function listSkillFiles(skillsDir) {
  ...
  return fs.readdirSync(skillsDir, { withFileTypes: true })
    .filter(entry => entry.isDirectory() && entry.name.startsWith('aexos-'))
...
function validateSkillPathConventions(content, filePath) {
  ...
  if (!content.includes('.aexos-core/development/agents/')) {
    errors.push(`${filePath} missing canonical source path ".aexos-core/development/agents/"`);
  }
  if (!content.includes('.aexos-core/development/scripts/generate-greeting.js')) {
