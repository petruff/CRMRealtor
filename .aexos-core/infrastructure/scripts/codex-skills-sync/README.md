...
Tools for generating local Codex skills (`.codex/skills/cyryx-*`) from AEXOS agent and squad definitions.
...
| Tool | Scope | Use case |
|------|-------|----------|
| `npm run sync:skills:codex` (`index.js`) | **Core agents only** (`.aexos-core/development/agents/*.md`) | CI / incremental sync. Tightly coupled with the shared `agent-parser.js` from `ide-sync/`. Used after editing core agent files to keep `.codex/skills/` in step. |
...
If a skill exists in `.codex/skills/` but does not contain the marker `<!-- CYRYX-CODEX-LOCAL-SKILLS: generated -->`, `bootstrap.js` skips it by default to preserve hand-edits. Pass `--force` to overwrite (a `.bak-<timestamp>` copy is created first).
...
1. **Core agents** — every `*.md` in `.aexos-core/development/agents/` becomes a `cyryx-<id>` skill.
...
3. **Skill ID** — `cyryx-<entry>` if the entry name is squad-specific (e.g. `mega-brain-chief`); otherwise `cyryx-<squad>-<entry>` to avoid collisions.
...
```
npm run validate:codex-skills           # validate cyryx-* skill / agent parity (strict)
npm run validate:codex-skills:self-test # parity + deterministic Skill tool self-test
```

Verifies that every core agent has a corresponding `.codex/skills/cyryx-<id>/SKILL.md`. Squad-chief skills are out of scope for this validator.
...
- The `<!-- CYRYX-CODEX-LOCAL-SKILLS: generated -->` marker (used by future runs to detect ownership)
- A pointer to the source-of-truth file (loaded on demand by Codex)
- A short activation protocol and starter command list

The source files in `.aexos-core/development/agents/` and `squads/*/agents/` remain authoritative. Generated skills are activator stubs.

## Story reference

Story 123.9 — Codex local skills bootstrap (squad chief coverage).

Background: students reported that squad-chief skills (e.g. `mega-brain-chief`, `slides-chief`) did not appear in Codex's `$` menu. Root cause: `index.js` only generated skills from `.aexos-core/development/agents/` — squad chiefs in `squads/*/agents/` were not covered. A provisional standalone script was distributed manually; this directory captures the definitive vendored solution.
