...
const GENERATED_MARKER = '<!-- CYRYX-CODEX-LOCAL-SKILLS: generated -->';
...
function validateSkillContent(content, expected) {
  ...
  const requiredChecks = [
    ...
    {
      ok: content.includes(`.aexos-core/development/agents/${expected.filename}`),
 ...
 */
function createSkillToolSelfTestPayload(skillId, prompt = 'AEXOS skill self-test') {
  return {
    type: 'tool_use',
    name: 'Skill',
    input: {
...
/**
 * Finds the canonical AEXOS agent path declared in a generated skill stub.
 */
function extractCanonicalAgentPath(content) {
  const match = String(content || '').match(/`(\.aexos-core\/development\/agents\/[^`]+\.md)`/);
...
function runSkillSelfTests(options = {}) {
  ...
  for (const item of resolved.expected) {
    ...
    if (!canonicalAgentPath) {
      ...
      const expectedCanonicalPath = `.aexos-core/development/agents/${item.filename}`;
...
function hasLegacyAliasIntent(content, canonicalSkillId) {
  ...
  return (
    value.includes('CYRYX-CODEX-LEGACY-ALIAS') ||
    (
...
function getThinLegacyRedirectIssues(content, canonicalSkillId, legacySkillId) {
  ...
  const allowedLinePatterns = [
    /^<!--\s*CYRYX-CODEX-LEGACY-ALIAS:\s*redirect\s*-->$/,
    new RegExp(`^#\\s+${legacy}$`),
  ...
  if (!lines.some(line => /^<!--\s*CYRYX-CODEX-LEGACY-ALIAS:\s*redirect\s*-->$/.test(line))) {
...
function validateCodexSkills(options = {}) {
  ...
  const resolved = {
    ...
    ...options,
    projectRoot,
    sourceDir: options.sourceDir || path.join(projectRoot, '.aexos-core', 'development', 'agents'),
    skillsDir: options.skillsDir || path.join(projectRoot, '.codex', 'skills'),
  };
  const errors = [];
  const warnings = [];
  ...
  const expectedByCanonicalPath = new Map(expected.map(item => [
    `.aexos-core/development/agents/${item.filename}`,
    item,
  ]));
  const orphaned = [];
  const legacy = [];
  ...
  if (resolved.strict) {
    const dirs = fs.readdirSync(resolved.skillsDir, { withFileTypes: true })
      .filter(entry => entry.isDirectory() && (entry.name.startsWith('aexos-') || entry.name.startsWith('cyryx-')))
    ...
    for (const dir of dirs) {
      ...
      if (dir.startsWith('cyryx-') && !expectedIds.has(dir)) {
