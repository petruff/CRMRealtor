 ...
 * @property {string} frameworkLocation - Path to AEXOS framework files
...
function detectRepositoryContext() {
  ...
  // Detect if we're in aexos-core repo itself
  const isFrameworkRepo =
    packageJson.name === '@cyryx/fullstack' ||
    packageJson.name === 'aexos-core' ||
    remoteUrl.includes('aexos-core');

  // Load installation config if exists
  let installConfig = null;
  const configPath = path.join(cwd, '.aexos-installation-config.yaml');
  ...
  return {
    ...
    projectRoot: cwd,
    frameworkLocation: isFrameworkRepo ? cwd : path.join(cwd, 'node_modules/@cyryx/fullstack'),
