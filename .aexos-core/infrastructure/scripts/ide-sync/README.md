...
Automatically synchronizes AEXOS agent definitions to IDE command files.

## Overview

IDE Sync keeps agent definitions in `.aexos-core/development/agents/` synchronized with IDE-specific command files in:

- `.claude/commands/AEXOS/agents/` (Claude Code)
- `.codex/agents/` (Codex CLI support files)
- `.gemini/rules/AEXOS/agents/` (Gemini CLI)
...
npm run sync:ide
# or
node .aexos-core/infrastructure/scripts/ide-sync/index.js sync
```
...
# or
node .aexos-core/infrastructure/scripts/ide-sync/index.js validate
```
...
# or
node .aexos-core/infrastructure/scripts/ide-sync/index.js validate --strict
```
...
Configure in `.aexos-core/core-config.yaml`:
...
```mdc
---
description: 'AEXOS redirect from @aexos-developer to @aexos-master'
alwaysApply: false
---

# Agent Redirect: aexos-developer -> aexos-master

> This agent has been renamed. Use `@aexos-master` instead.
```
