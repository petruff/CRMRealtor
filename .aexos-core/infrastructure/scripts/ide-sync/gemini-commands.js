...
const FALLBACK_DESCRIPTION = 'Agente especializado AEXOS';
...
function commandSlugForAgent(agentId) {
  if (agentId.startsWith('aexos-')) {
    return agentId.replace(/^aexos-/, '');
  }
  return agentId;
}

function menuCommandName(agentId) {
  return `/aexos-${commandSlugForAgent(agentId)}`;
}
...
function buildAgentDescription(agent) {
  ...
  return `Ativar agente AEXOS ${agent.id}`;
}

function buildAgentCommandPrompt(agentId) {
  return [
    `Ative o agente ${agentId}:`,
    `1. Leia a definição completa em .gemini/rules/AEXOS/agents/${agentId}.md`,
    '2. Siga as activation-instructions do bloco YAML',
    `3. Renderize o greeting via: node .aexos-core/development/scripts/generate-greeting.js ${agentId}`,
...
function buildMenuPrompt(commandFiles) {
  const lines = [
    'Você está no launcher AEXOS para Gemini.',
    '',
...
function buildMenuCommandFile(commandFiles) {
  const content = [
    'description = "Menu rápido AEXOS (lista agentes e orienta qual ativar)"',
    'prompt = """',
