 ...
 * Target: .kimi/skills/aexos-{agent-id}/SKILL.md
 */
...
function transform(agentData) {
  ...
  const title = rawAgent.title || fallbackAgent.title || 'AEXOS Agent';
...
function buildDescription(id, name, title, whenToUse) {
  ...
  const brandedTitle = /^AEXOS\b/i.test(title) ? title : `AEXOS ${title}`;
...
function getSkillId(agentData) {
  ...
  if (id.startsWith('aexos-')) return id;
  if (id.startsWith('cyryx-')) return id.replace(/^cyryx-/, 'aexos-');
  return `aexos-${id}`;
}
