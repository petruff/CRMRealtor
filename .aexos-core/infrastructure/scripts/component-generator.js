/**
 * Component Generator for AEXOS (Cyryx)
...
class ComponentGenerator {
  constructor(options = {}) {
    ...
    this.templatesPath = path.join(this.rootPath, 'aexos-core', 'templates');
    this.outputPaths = {
      agent: path.join(this.rootPath, 'aexos-core', 'agents'),
      task: path.join(this.rootPath, 'aexos-core', 'tasks'),
      workflow: path.join(this.rootPath, 'aexos-core', 'workflows'),
    };
  ...
  processAnswersToVariables(componentType, answers) {
    ...
    switch (componentType) {
      ...
      case 'task':
        ...
        // Notes
        variables.EACH_NOTES = ['Generated using AEXOS (Cyryx) template system'];
        break;
      ...
      case 'workflow':
        ...
        // Metadata
        variables.AUTHOR = process.env.USER || 'AEXOS (Cyryx)';
  ...
  async createAgentCommandTasks(variables, options) {
    ...
    for (const command of variables.COMMANDS) {
      const taskId = this.commandToTaskId(command.COMMAND_NAME);
      const taskPath = path.join(this.rootPath, 'aexos-core', 'tasks', `${taskId}.md`);
