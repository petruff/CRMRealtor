...
function validateClaudeIntegration(options = {}) {
  ...
  const agentsDir = options.agentsDir || path.join(projectRoot, '.claude', 'commands', 'AEXOS', 'agents');
  const skillsAgentsDir =
    options.skillsAgentsDir || path.join(projectRoot, '.claude', 'skills', 'AEXOS', 'agents');
  ...
  const sourceAgentsDir =
    options.sourceAgentsDir || path.join(projectRoot, '.aexos-core', 'development', 'agents');
  ...
  const disallowedAgentMemoryEntries = agentMemoryEntries.filter((entry) => !entry.startsWith('aexos-'));
