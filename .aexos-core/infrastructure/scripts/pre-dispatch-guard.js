 ...
 */
function run(env = process.env, projectRoot = env.AEXOS_PROJECT_ROOT || process.cwd()) {
  const contextFile = env.AEXOS_DISPATCH_CONTEXT || '';
  const context = loadContext(contextFile);
  const task = env.AEXOS_DISPATCH_TASK || '';
  const params = env.AEXOS_DISPATCH_PARAMS || '';
  const story =
    context.storyPath || context.story || context.storyId || context.metadata?.storyPath || params;
  const intent = JSON.stringify({
    agent: env.AEXOS_DISPATCH_AGENT || '',
    task,
    params,
    context,
  });
  return assertDispatchGovernance({
    budgetCeilingUsd: env.AEXOS_MODEL_BUDGET_CEILING_USD,
    task,
    intent,
    story,
