...
const GitignoreTemplates = {
  AEXOS_BASE: 'gitignore-aexos-base.tmpl',
...
function getTemplatesForStacks(techStacks) {
  const templates = [GitignoreTemplates.AEXOS_BASE];
...
function generateGitignore(markers, options = {}) {
  ...
  sections.push('# Generated by AEXOS Documentation Integrity System');
...
/**
 * Merges AEXOS ignores with existing .gitignore
 *
...
function mergeGitignore(existingContent, _options = {}) {
  // Check if AEXOS section already exists
  if (existingContent.includes('AEXOS Integration Section')) {
    console.log('AEXOS section already exists in .gitignore, skipping merge');
...
# AEXOS Local Configuration
.aexos-core/local/
.aexos-core/*.local.yaml
.aexos-core/logs/
.aexos-core/cache/
...
/**
 * Checks if a .gitignore file has AEXOS integration
 *
 * @param {string} targetDir - Target directory
 * @returns {boolean} True if AEXOS section exists
 */
function hasCyryxIntegration(targetDir) {
  const gitignorePath = path.join(targetDir, '.gitignore');
  ...
  return content.includes('AEXOS Integration Section') || content.includes('.aexos-core/');
}
...
function parseGitignore(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('AEXOS Integration Section')) {
      if (cyryxStart === -1) {
        cyryxStart = i;
      }
    }

    if (line.includes('End of AEXOS Integration Section')) {
      cyryxEnd = i;
    }

    if (line.startsWith('#') && !line.includes('AEXOS')) {
  ...
  if (cyryxStart !== -1) {
    sections.cyryxSection = {
      start: cyryxStart,
      end: cyryxEnd !== -1 ? cyryxEnd : lines.length - 1,
    };
  }
