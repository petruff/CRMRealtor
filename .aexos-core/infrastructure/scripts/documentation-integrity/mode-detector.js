 ...
 *
 * Detects installation mode for AEXOS projects with three-mode support:
...
/**
 * Installation modes supported by AEXOS
 * @enum {string}
 */
...
const ModeDescriptions = {
  [InstallationMode.FRAMEWORK_DEV]: {
    ...
    hint: 'Developing aexos-core itself - uses framework standards, skips project setup',
    description: 'For AEXOS contributors working on the framework',
  },
  [InstallationMode.GREENFIELD]: {
    label: '🆕 New Project (Greenfield)',
    hint: 'Start a fresh project with AEXOS - generates project docs, config, and infrastructure',
    description: 'Empty directory setup with full scaffolding',
  },
  [InstallationMode.BROWNFIELD]: {
    label: '📂 Existing Project (Brownfield)',
    hint: 'Add AEXOS to existing project - analyzes current structure and adapts',
 ...
 * 1. FRAMEWORK_DEV - .aexos-core/ exists AND is aexos-core repo
...
function detectInstallationMode(targetDir = process.cwd()) {
  ...
  // Priority 1: Check for AEXOS framework development
  if (markers.hasCyryxCore && markers.isCyryxCoreRepo) {
    return {
      mode: InstallationMode.FRAMEWORK_DEV,
      legacyType: LegacyProjectType.EXISTING_CYRYX,
      confidence: 100,
      reason: 'Detected aexos-core repository with .aexos-core directory',
      markers,
    };
  }

  // Priority 2: Check for existing AEXOS installation (non-framework)
  if (markers.hasCyryxCore && !markers.isCyryxCoreRepo) {
    return {
      mode: InstallationMode.BROWNFIELD,
      legacyType: LegacyProjectType.EXISTING_CYRYX,
      confidence: 95,
      reason: 'AEXOS already installed in user project - treating as brownfield update',
      markers,
    };
  }
...
function collectMarkers(targetDir) {
  ...
  return {
    // AEXOS markers
    hasCyryxCore: fs.existsSync(path.join(targetDir, '.aexos-core')),
    isCyryxCoreRepo: isCyryxCoreRepository(targetDir),
...
/**
 * Checks if the target directory is the aexos-core repository itself
 *
 * @param {string} targetDir - Directory to check
 * @returns {boolean} True if this is the aexos-core repository
 */
function isCyryxCoreRepository(targetDir) {
  ...
  try {
    ...
    // Primary check: explicit aexos-core package names
    if (packageJson.name === '@cyryx/core' || packageJson.name === 'aexos-core') {
      return true;
    }

    // Secondary check: workspaces pattern + aexos-specific marker file
    // This prevents false positives for generic monorepos
    const hasCyryxMarker = fs.existsSync(path.join(targetDir, '.aexos-core', 'infrastructure'));
    const hasWorkspaces =
      Array.isArray(packageJson.workspaces) && packageJson.workspaces.includes('packages/*');

    return hasWorkspaces && hasCyryxMarker;
  } catch (error) {
    // Log error for debugging but don't throw - return false for safety
    if (process.env.AEXOS_DEBUG) {
      console.warn(`[mode-detector] Error checking aexos-core repository: ${error.message}`);
    }
    return false;
  }
}
 ...
 * for EXISTING_CYRYX legacy type (both use EXISTING_CYRYX but with different modes).
 * For accurate mode detection, use the mode property from detectInstallationMode().
 *
 * @param {string} legacyType - Legacy project type (EXISTING_CYRYX, GREENFIELD, etc.)
 * @param {Object} [context] - Optional context for disambiguation
 * @param {boolean} [context.isCyryxCoreRepo] - True if this is the aexos-core repository
 * @returns {string} New installation mode
 */
function mapLegacyTypeToMode(legacyType, context = {}) {
  // EXISTING_CYRYX is ambiguous: it can be FRAMEWORK_DEV (aexos-core repo)
  // or BROWNFIELD (user project with AEXOS installed)
  if (legacyType === LegacyProjectType.EXISTING_CYRYX) {
    // Use context to disambiguate if provided
    if (context.isCyryxCoreRepo === true) {
      return InstallationMode.FRAMEWORK_DEV;
    }
    if (context.isCyryxCoreRepo === false) {
...
function validateModeSelection(selectedMode, detected) {
  ...
  if (selectedMode !== detected.mode) {
    ...
    if (selectedMode === InstallationMode.FRAMEWORK_DEV && !detected.markers.isCyryxCoreRepo) {
      result.warnings.push(
        'Selected framework-dev but this does not appear to be the aexos-core repository.',
      );
    }
