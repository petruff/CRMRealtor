...
1. Read source of truth if deep task execution is needed: \`.aexos-core/development/agents/${agentData.filename}\`
2. Greet briefly:
   - ${greeting}
...
2. **Source of truth** for full commands/tasks: \`.aexos-core/development/agents/${agentData.filename}\`
...
function buildRulesMarkdown() {
  return `# AEXOS × Grok — Compact Rules
...
`;
  ...
  // Rules + README
  const extras = [
    { path: resolveUnder(targets.rules, 'aexos-core.md'), content: buildRulesMarkdown() },
