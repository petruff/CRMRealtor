 ...
 *
 * Features:
 * - AC1: Located in `.aexos-core/infrastructure/scripts/`
 ...
 * - AC7: Integrates with `.aexos/dashboard/status.json`
 *
...
class PlanTracker {
  ...
  _findPlanPath() {
    const searchPaths = [
      ...
      // .aexos plans location
      path.join(this.rootPath, '.aexos/plans', this.storyId, CONFIG.implementationFile),
      // Epic-based location (e.g., aexos-core-ade/story-4.6)
   ...
   *
   * Updates `.aexos/dashboard/status.json` for dashboard integration.
   * Also updates legacy `.aexos/status.json` for backwards compatibility.
   *
...
async function main() {
  ...
  if (args.length < 1 || args.includes('--help') || args.includes('-h')) {
    console.log(`
\u{1F4CA} Plan Progress Tracker - AEXOS Execution Engine (Story 4.6)
