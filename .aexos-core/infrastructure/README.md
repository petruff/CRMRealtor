...
Base layer of the AEXOS modular architecture. Contains tools, integrations, scripts, and PM adapters.
...
- `ComponentGenerator` - AEXOS component generation
- `BatchCreator` - Batch operations

### Validation
- `CyryxValidator` - AEXOS component validation
...
const {
  ...
  TemplateEngine,
  resolveTool
} = require('.aexos-core/infrastructure');

// Or import directly from scripts
const GitWrapper = require('.aexos-core/infrastructure/scripts/git-wrapper');
```

## Tool Resolution

```javascript
const { resolveTool } = require('.aexos-core/infrastructure');
...
```javascript
const { getPMAdapter, isPMToolConfigured } = require('.aexos-core/infrastructure');
