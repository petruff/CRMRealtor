...
Este documento define os princípios fundamentais e inegociáveis do AEXOS (Cyryx). Todos os agentes, tasks, e workflows DEVEM respeitar estes princípios. Violações são bloqueadas automaticamente via gates.
...
**Rationale:** Artifacts that live only under one IDE folder are host-locked. AEXOS is runtime-agnostic by design — value is in the process, not the IDE.

**OSS note:** Framework core agents live under `.aexos-core/development/`; squad expansions use `squads/`. Both remain portable across IDEs via sync scripts.
...
- **Gates implementados em:** `.aexos-core/development/tasks/`
- **Checklists relacionados:** `.aexos-core/product/checklists/`

---

*AEXOS (Cyryx) Constitution v1.1.0*
