/**
 * CLI: aexos wave — lean wave-execute plan / status
 *
 * Usage:
 *   aexos wave plan --stories <p1,p2,...> [--wave-id ID] [--mode yolo] [--save]
 *   aexos wave plan --glob <glob> ...
 *   aexos wave status <wave-id>
...
/**
 * Build the `aexos wave` Commander command tree.
 *
...
function createWaveCommand() {
  ...
  cmd
    ...
    .action((opts) => {
      try {
        ...
        console.log('Next: aexos wave advance ' + plan.waveId);
      } catch (err) {
  ...
  cmd
    ...
    .option('--wave-id <id>', 'Wave id')
    .option('--mode <mode>', 'yolo | interactive', 'interactive')
    .option('--save', 'Persist under .aexos/waves/', false)
    .option('--json', 'JSON output', false)
    .action((opts) => {
      const list = [];
      ...
      for (const b of plan.batches) {
        ...
        for (const s of b.stories) {
          ...
          console.log(`    ${s.path}`);
          console.log(`    → full-sdc: aexos sdc plan ${s.path} --mode ${plan.mode}`);
        }
      }
      if (opts.save || opts.waveId) {
        console.log(`\nSaved: ${sdc.waveStatePath(plan.waveId)}`);
      }
      console.log('\nDispatch each story with skill full-sdc (or aexos sdc next).');
  ...
  cmd
    ...
    .action((waveId, opts) => {
      ...
      if (!nextBatch) {
        ...
        console.log(`Optional: aexos wave report ${waveId}`);
        return;
      }
  ...
  cmd
    ...
    .action((waveId, opts) => {
      try {
        ...
        if (!nextBatch) {
          console.log('No open batches. aexos wave report ' + waveId);
          return;
        }
  ...
  cmd
    .command('report')
    .description('Write .aexos/waves/{id}/report.md')
    .argument('<wave-id>', 'Wave id')
    .option('--json', 'JSON output', false)
    .action((waveId, opts) => {
      try {
