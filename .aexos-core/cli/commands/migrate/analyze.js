...
async function detectV2Structure(projectRoot) {
  ...
  if (!fs.existsSync(cyryxCoreDir)) {
    return {
      ...
      isV21: false,
      version: null,
      error: 'No .aexos-core directory found',
    };
  }
  ...
  const hasV21Structure = v21Modules.every(module =>
    fs.existsSync(path.join(cyryxCoreDir, module)),
  );
  ...
  const hasV20Structure = v20Indicators.some(dir =>
    fs.existsSync(path.join(cyryxCoreDir, dir)),
  );
  ...
  return {
    ...
    isV21: false,
    version: null,
    error: 'Unable to detect AEXOS version structure',
  };
}

/**
 * Categorize file into target module
 * @param {string} relativePath - Relative path from .aexos-core
...
async function analyzeMigrationPlan(projectRoot, options = {}) {
  ...
  // Get all files
  const allFiles = await getAllFiles(cyryxCoreDir);
  ...
  for (const filePath of allFiles) {
    const relativePath = path.relative(cyryxCoreDir, filePath);
    ...
    if (module && plan.modules[module]) {
      ...
      fileInfo.targetPath = path.join(cyryxCoreDir, module, relativePath);
  ...
  for (const moduleName of Object.keys(plan.modules)) {
    const targetDir = path.join(cyryxCoreDir, moduleName);
