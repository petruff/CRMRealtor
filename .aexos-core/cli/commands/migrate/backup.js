...
/**
 * Create backup of .aexos-core directory
...
async function createBackup(projectRoot, options = {}) {
  ...
  if (!fs.existsSync(cyryxCoreDir)) {
    throw new Error('No .aexos-core directory found. Is this an AEXOS v2.0 project?');
  }
  ...
  const files = await getAllFiles(cyryxCoreDir);
  const manifest = {
    version: '2.0',
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const relativePath = path.relative(cyryxCoreDir, srcFile);
    const destFile = path.join(backupDir, '.aexos-core', relativePath);
...
async function verifyBackup(backupDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    throw new Error('Backup manifest not found. Is this a valid AEXOS backup?');
  }
  ...
  for (const file of manifest.files) {
    const filePath = file.isConfig
      ...
      : path.join(backupDir, '.aexos-core', file.relativePath);
...
async function findLatestBackup(projectRoot) {
  ...
  const backups = entries
    .filter(entry => entry.isDirectory() && entry.name.startsWith('.aexos-backup-'))
    .map(entry => ({
      name: entry.name,
      path: path.join(projectRoot, entry.name),
      date: entry.name.replace('.aexos-backup-', ''),
    }))
...
async function listBackups(projectRoot) {
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && entry.name.startsWith('.aexos-backup-')) {
