 ...
 */
async function validateStructure(cyryxCoreDir) {
  const result = {
    valid: true,
    modules: {},
    errors: [],
  ...
  const rootEntries = await fs.promises.readdir(cyryxCoreDir, { withFileTypes: true });
...
async function runFullValidation(projectRoot, options = {}) {
  ...
  result.structure = await validateStructure(cyryxCoreDir);
  ...
  result.imports = await verifyImports(cyryxCoreDir);
...
function generateSummary(migrationResult, validationResult, options = {}) {
  ...
  lines.push('  3. Report issues: github.com/cyryx/issues');
  lines.push('');
  lines.push('To rollback: aexos migrate --rollback');
