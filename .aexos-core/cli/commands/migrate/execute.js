 ...
 */
async function createModuleDirectories(cyryxCoreDir, options = {}) {
 ...
 */
async function migrateModule(moduleData, moduleName, cyryxCoreDir, options = {}) {
...
async function executeMigration(plan, options = {}) {
  ...
  if (!dryRun) {
    await createModuleDirectories(plan.cyryxCoreDir, { onProgress });
  }
  ...
  if (plan.uncategorized.length > 0 && !dryRun) {
    ...
    for (const file of plan.uncategorized) {
      try {
        const targetPath = path.join(plan.cyryxCoreDir, 'core', file.relativePath);
        await copyFileWithMetadata(file.sourcePath, targetPath);
        result.totalFiles++;
        result.totalSize += file.size;
      } catch (error) {
  ...
  if (!dryRun && cleanupOriginals) {
    ...
    for (const [_moduleName, config] of Object.entries(MODULE_MAPPING)) {
      for (const dir of config.directories) {
        const originalDir = path.join(plan.cyryxCoreDir, dir);
...
async function saveMigrationState(projectRoot, state) {
  const statePath = path.join(projectRoot, '.aexos-migration-state.json');
