...
function printHeader(fromVersion, toVersion) {
  console.log('');
  console.log(`🔄 AEXOS Migration v${fromVersion} → v${toVersion}`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
}
...
async function runMigration(options) {
  ...
  if (!versionInfo.isV2) {
    ...
    console.error(`❌ ${versionInfo.error || 'Cannot detect AEXOS version'}`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (!migrationResult.success) {
      ...
      console.log('  Run: aexos migrate --rollback');
      process.exit(1);
    }
  ...
  try {
    ...
    const importResult = await updateAllImports(cyryxCoreDir, plan, { verbose, onProgress });
  ...
  const backupDir = `.aexos-backup-${new Date().toISOString().split('T')[0]}`;
...
async function runRollbackCommand(options) {
  ...
  console.log('');
  console.log('🔙 AEXOS Migration Rollback');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
...
async function showBackupStatus(_options) {
  ...
  console.log('');
  console.log('📦 AEXOS Migration Backups');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
...
function createMigrateCommand() {
  ...
  migrate
    .description('Migrate AEXOS from v2.0 to v4.0.4 modular structure')
...
Migration Flow:
  Phase 1: Backup
    - Creates .aexos-backup-{date}/ with all files
...
Examples:
  $ aexos migrate                          # Interactive migration
  $ aexos migrate --dry-run                # Preview changes
  $ aexos migrate --verbose                # Detailed output
  $ aexos migrate -y --skip-tests          # Non-interactive, skip tests
  $ aexos migrate --rollback               # Restore from backup
  $ aexos migrate --status                 # Show backups

Rollback:
  If migration fails or you want to revert:
  $ aexos migrate --rollback

  Manual rollback:
  $ cp -r .aexos-backup-{date}/* .aexos-core/
`);
