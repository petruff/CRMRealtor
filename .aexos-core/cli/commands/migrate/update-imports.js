...
function transformImportPath(importPath, currentFileModule, currentFilePath, transformMap) {
  ...
  if (importPath.startsWith('../') || importPath.startsWith('./')) {
    ...
    // 1. If the path goes up to .aexos-core root and then into a folder, update it
    ...
    // If going up enough to reach .aexos-core root
    if (restParts.length > 0) {
      ...
      for (const [moduleName, config] of Object.entries(MODULE_MAPPING)) {
        if (config.directories.includes(targetDir)) {
          ...
          // Current structure after migration: .aexos-core/{module}/...
          // Need to go to: .aexos-core/{targetModule}/...
  ...
  try {
    ...
    for (const imp of imports) {
      const relativePath = path.relative(
        path.dirname(filePath).split('.aexos-core')[1]?.slice(1) || '',
        '',
      );
 ...
 * @param {string} cyryxCoreDir - Path to .aexos-core
 * @param {Object} plan - Migration plan
 * @param {Object} options - Options
 * @returns {Promise<Object>} Update results
 */
async function updateAllImports(cyryxCoreDir, plan, options = {}) {
  ...
  for (const moduleName of modules) {
    const moduleDir = path.join(cyryxCoreDir, moduleName);
    ...
    for (const file of files) {
      ...
      if (verbose && updateResult.updated > 0) {
        ...
        onProgress({
          phase: 'file',
          message: `  → ${path.relative(cyryxCoreDir, file)} (${updateResult.updated} imports)`,
        });
      }
    }
  }
 ...
 */
async function verifyImports(cyryxCoreDir) {
  const result = {
    valid: true,
    totalImports: 0,
    brokenImports: [],
        ...
        if (imp.importPath.startsWith('.')) {
          ...
          if (!found) {
            result.brokenImports.push({
              file: path.relative(cyryxCoreDir, file),
