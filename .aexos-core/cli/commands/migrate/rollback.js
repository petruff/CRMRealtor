 ...
 */
async function removeV21Structure(cyryxCoreDir, options = {}) {
...
async function restoreFromBackup(backup, projectRoot, options = {}) {
  ...
  const backupCyryxCoreDir = path.join(backup.path, '.aexos-core');

  // Restore .aexos-core files
  ...
  for (const file of backup.manifest.files) {
    try {
      const sourcePath = file.isConfig
        ...
        : path.join(backupCyryxCoreDir, file.relativePath);

      const targetPath = file.isConfig
        ? path.join(projectRoot, file.relativePath)
        : path.join(cyryxCoreDir, file.relativePath);
...
async function executeRollback(projectRoot, options = {}) {
  ...
  const cyryxCoreDir = path.join(projectRoot, '.aexos-core');
  result.removal = await removeV21Structure(cyryxCoreDir, { onProgress });
  ...
  for (const dir of expectedDirs) {
    if (fs.existsSync(path.join(cyryxCoreDir, dir))) {
      validCount++;
    }
  }
...
function formatRollbackSummary(result) {
  ...
  lines.push('');
  lines.push('🔙 AEXOS Migration Rollback');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
