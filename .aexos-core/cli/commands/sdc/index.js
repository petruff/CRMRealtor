/**
 * CLI: aexos sdc — lean full-sdc plan / status / verify / mark
 *
 * Usage:
 *   aexos sdc plan <story-path> [--mode yolo|interactive] [--force]
 *   aexos sdc status <story-id|story-path>
 *   aexos sdc verify <story-path> <phase> [--mark]
 *   aexos sdc mark <story-id> <phase> --status passed|failed|skipped|halted [--notes "..."]
 *   aexos sdc phases
 */
...
/**
 * Build the `aexos sdc` Commander command tree.
 *
...
function createSdcCommand() {
  ...
  cmd
    ...
    .action((storyPath, opts) => {
      ...
      console.log('\nNext: run skill phases, then: aexos sdc verify <story> <phase> --mark');
    });
  ...
  cmd
    ...
    .action((story, opts) => {
      ...
      if (!state) {
        console.error(`No SDC state for ${storyId}. Run: aexos sdc plan <story-path>`);
        process.exitCode = 1;
        return;
      }
      if (opts.json) {
  ...
  cmd
    ...
    .action((storyId, phase, opts) => {
      ...
      if (!state) {
        console.error(`No state for ${storyId}; run aexos sdc plan first`);
        process.exitCode = 1;
        return;
      }
      if (
  ...
  cmd
    ...
    .action((story, opts) => {
      ...
      console.log(`  after:  aexos sdc verify ${state.storyPath} ${phase} --mark`);
    });
