#!/bin/bash
#
# AEXOS Framework Documentation Migration Script
#
# Purpose: Migrate framework docs from aexos-fullstack to aexos-core (REPO 1)
...
#   --target-repo    Path to aexos-core repository (default: ../aexos-core)
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  AEXOS Framework Documentation Migration Script             ║${NC}"
...
if [ ! -d "$SOURCE_DOCS" ]; then
  ...
  echo "  Please run this script from the aexos-fullstack root directory"
  exit 1
fi
...
if [ ! -d "$TARGET_REPO" ]; then
  ...
  echo "  Please clone aexos-core first or specify correct path with --target-repo"
  exit 1
fi
...
**Source:** aexos-fullstack/docs/framework/
**Target:** aexos-core/docs/framework/
**Story:** 6.1.2.6 - Framework Configuration System

## Migration Status

✅ Framework documentation successfully migrated from aexos-fullstack to aexos-core (REPO 1)

## Files Migrated

- coding-standards.md - Official AEXOS coding standards
...
- aexos-fullstack maintains copies in `docs/framework/` (will be removed Q3 2026)
- aexos-fullstack maintains legacy copies in `docs/architecture/` (will be removed Q3 2026)
...
if [ "$DRY_RUN" = false ] && [ -d "$TARGET_REPO/.git" ]; then
  ...
  echo "    git commit -m \"docs: migrate framework documentation from aexos-fullstack\""
  echo ""
  echo "    Story: 6.1.2.6 - Framework Configuration System"
  echo "    Migrated: coding-standards.md, tech-stack.md, source-tree.md"
  echo "    Source: aexos-fullstack/docs/framework/"
  echo ""
...
echo "    4. Commit changes to aexos-core repository"
echo "    5. Update aexos-fullstack deprecation warnings (Q3 2026)"
echo ""
