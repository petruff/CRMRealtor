#!/bin/bash
# ============================================
# AEXOS PM Orchestration Script (pm.sh)
...
#   --output    - Custom output file path (default: /tmp/aexos-output-{timestamp}.md)
#
# Environment Variables:
#   AEXOS_OUTPUT_DIR   - Directory for output files (default: /tmp)
#   AEXOS_DEBUG        - Enable debug logging (default: false)
#   AEXOS_TIMEOUT      - Timeout in seconds (default: 300)
#   CLAUDE_CMD        - Claude CLI command (default: claude)
#   AEXOS_MODEL_BUDGET_CEILING_USD - Required positive budget ceiling
#
# Exit Codes:
#   0 - Success
#   1 - Invalid arguments
...
# Configuration
OUTPUT_DIR="${AEXOS_OUTPUT_DIR:-/tmp}"
DEBUG="${AEXOS_DEBUG:-false}"
TIMEOUT="${AEXOS_TIMEOUT:-300}"
CLAUDE_CMD="${CLAUDE_CMD:-claude}"
INLINE_MODE="${AEXOS_INLINE_MODE:-false}"
...
show_help() {
  cat << EOF
AEXOS Multi-Modal Orchestration Script v${VERSION}
...
Environment Variables:
  AEXOS_OUTPUT_DIR    Output directory (default: /tmp)
  AEXOS_DEBUG         Enable debug mode (default: false)
  AEXOS_TIMEOUT       Timeout in seconds (default: 300)
  AEXOS_INLINE_MODE   Run without a visual terminal (default: false)
  AEXOS_NO_VISUAL_TERMINAL
                     Never open a visual terminal, force inline (default: false;
                     also implied by JEST_WORKER_ID, NODE_ENV=test, or CI=true)
  CLAUDE_CMD         Claude CLI command (default: claude)
  AEXOS_MODEL_BUDGET_CEILING_USD
...
setup_paths() {
  ...
  if [[ -n "$CUSTOM_OUTPUT" ]]; then
    ...
    OUTPUT_FILE="${OUTPUT_DIR}/aexos-output-${timestamp}.md"
  fi

  LOCK_FILE="${OUTPUT_DIR}/aexos-lock-${timestamp}.lock"
...
build_agent_prompt() {
  local prompt=""
  prompt+="You are the AEXOS agent @${AGENT}. Execute task *${TASK}"
  [[ -n "$PARAMS" ]] && prompt+=" ${PARAMS}"
  prompt+=". Follow .aexos-core/constitution.md and agent authority rules. "
  prompt+="Load persona from .aexos-core/development/agents/ when present. "
  if [[ -n "$CONTEXT_FILE" && -f "$CONTEXT_FILE" ]]; then
...
run_dispatch_guard() {
  ...
  AEXOS_DISPATCH_AGENT="$AGENT" \
    AEXOS_DISPATCH_TASK="$TASK" \
    AEXOS_DISPATCH_PARAMS="$PARAMS" \
    AEXOS_DISPATCH_CONTEXT="$CONTEXT_FILE" \
    AEXOS_PROJECT_ROOT="${AEXOS_PROJECT_ROOT:-$(pwd)}" \
    node "$guard_script" >&2
}
...
run_agent_cli() {
  ...
  {
    echo "=== AEXOS Agent Session (${mode_label}) ==="
    echo "Agent: ${AGENT}"
    echo "Task: ${TASK}"
...
spawn_terminal() {
  ...
  # explicit AEXOS_NO_VISUAL_TERMINAL opt-out must never open a real terminal.
  if [[ "$INLINE_MODE" == "true" || -n "${JEST_WORKER_ID:-}" || "${NODE_ENV:-}" == "test" || "${CI:-false}" == "true" || "${AEXOS_NO_VISUAL_TERMINAL:-false}" == "true" ]]; then
    local inline_result=0
  ...
  full_cmd+="AEXOS_INLINE_MODE=true AEXOS_OUTPUT_DIR=${quoted} "
  printf -v quoted '%q' "$DEBUG"
  full_cmd+="AEXOS_DEBUG=${quoted} "
  printf -v quoted '%q' "$CLAUDE_CMD"
  full_cmd+="CLAUDE_CMD=${quoted} "
  printf -v quoted '%q' "${AEXOS_MODEL_BUDGET_CEILING_USD:-}"
  full_cmd+="AEXOS_MODEL_BUDGET_CEILING_USD=${quoted} "
...
main() {
  ...
  log_info "AEXOS Multi-Modal Orchestration Script v${VERSION}"
