#!/bin/bash
# AEXOS Framework Update - v5.2 (Optimized)
#
# LOGIC:
...
#
# Usage: bash .aexos-core/scripts/update-aexos.sh

set -e

echo "⚡ AEXOS Update v5.2"
echo ""
...
# Validate: clean working tree for .aexos-core
if [ -n "$(git status --porcelain .aexos-core/)" ]; then
  echo "❌ Commit .aexos-core changes first:"
  git status --short .aexos-core/
  echo ""
  echo "Run: git add .aexos-core && git commit -m 'your message'"
  exit 1
fi
...
git clone --depth 1 --filter=blob:none --sparse \
  https://github.com/CyryxLabs/AEXOS.git \
...
git sparse-checkout set .aexos-core
cd - > /dev/null

echo "✅ Fetched upstream"
echo ""

# Build file lists (relative paths without .aexos-core/ prefix)
echo "📋 Scanning files..."
find .aexos-core -type f | sed 's|^\.aexos-core/||' | sort > "$TEMP_DIR/local-files.txt"
find "$TEMP_DIR/upstream/.aexos-core" -type f | sed "s|^$TEMP_DIR/upstream/\.aexos-core/||" | sort > "$TEMP_DIR/upstream-files.txt"

# Get git-tracked files (for DELETE detection) - ONE call instead of N calls
git ls-files .aexos-core | sed 's|^\.aexos-core/||' | sort > "$TEMP_DIR/tracked-files.txt"
...
while IFS= read -r rel_path; do
  if ! cmp -s ".aexos-core/$rel_path" "$TEMP_DIR/upstream/.aexos-core/$rel_path"; then
...
while IFS= read -r rel_path; do
  ...
  cp -a ".aexos-core/$rel_path" "$TEMP_DIR/local-only/$rel_path"
...
if [ -s "$REPORT_DELETED" ]; then
  while IFS= read -r rel_path; do
    rm -f ".aexos-core/$rel_path"
  done < "$REPORT_DELETED"
fi

# Copy all upstream files (creates new + overwrites existing)
rsync -a "$TEMP_DIR/upstream/.aexos-core/" ".aexos-core/"

# Restore local-only files
if [ -d "$TEMP_DIR/local-only" ] && [ "$(ls -A "$TEMP_DIR/local-only" 2>/dev/null)" ]; then
  rsync -a "$TEMP_DIR/local-only/" ".aexos-core/"
fi

# Clean empty directories
find .aexos-core -type d -empty -delete 2>/dev/null || true
...
echo ""
echo "Choose:"
echo "  ✅ Apply:  git add .aexos-core && git commit -m 'chore: sync AEXOS framework'"
echo "  ❌ Cancel: git checkout -- .aexos-core/"
echo ""
