/**
 * Verification Tests for Active AEXOS Memory Modules
 *
...
test('merge-rules.yaml exists in .cyryx', () => {
  const rulesPath = path.join(process.cwd(), '.aexos', 'merge-rules.yaml');
  const exists = fs.existsSync(rulesPath);

  assertTrue(exists, 'merge-rules.yaml should exist in .cyryx');
});
