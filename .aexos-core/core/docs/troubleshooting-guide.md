# AEXOS (Cyryx) Meta-Agent Troubleshooting Guide

## Overview

This guide helps diagnose and resolve common issues when using the AEXOS (Cyryx) meta-agent for component creation and management.
...
**Symptoms:**
```
❌ Error: Agent 'data-analyst' already exists at /aexos-core/agents/data-analyst.md
```
...
3. If overwriting is intended:
   ...
   rm aexos-core/agents/data-analyst.md
   *create-agent
   ```
...
1. Create session directory:
   ```bash
   mkdir -p aexos-core/.sessions
   ```

2. Check permissions:
   ```bash
   chmod 755 aexos-core/.sessions
   ```
...
2. Check transaction directory:
   ```bash
   ls aexos-core/logs/transactions/
   ```
...
1. Manual cleanup:
   ```bash
   # Check failed files
   ls -la aexos-core/agents/file1.md
   
   # Remove manually if needed
   rm aexos-core/agents/file1.md
   ```
...
3. Restore from backup:
   ```bash
   # Check backups
   ls aexos-core/logs/transactions/txn-*/backups/
   ```
...
1. Check failure details:
   ...
   cat aexos-core/logs/transactions/latest.json
   ```
...
# Specific modules
DEBUG=cyryx:template,cyryx:elicitation *create-agent
...
```bash
# Application logs
tail -f aexos-core/logs/aexos-developer.log

# Transaction logs
ls -la aexos-core/logs/transactions/

# Error logs
grep ERROR aexos-core/logs/*.log
```
...
- Template syntax: `aexos-core/docs/template-syntax.md`
- Creation guide: `aexos-core/docs/component-creation-guide.md`
- API reference: `aexos-core/docs/api-reference.md`
...
1. **Backup current state**:
   ```bash
   tar -czf aexos-backup.tar.gz aexos-core/
   ```

2. **Reset to clean state**:
   ```bash
   git checkout -- aexos-core/
   npm run setup
   ```
