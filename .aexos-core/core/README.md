...
> Central runtime module providing essential framework functionality for AEXOS (Cyryx).
...
The Core module contains the foundational runtime components that all other AEXOS modules depend on. It provides configuration management, session handling, elicitation workflows, and essential utilities.

## Installation

The core module is automatically available within the AEXOS (Cyryx) framework:
...
// ES Modules
import { ConfigCache, ElicitationEngine } from './.aexos-core/core/index.esm.js';
```
...
│   ├── aexos-kb.md          # AEXOS knowledge base
...
```javascript
const { globalConfigCache } = require('./.aexos-core/core');
...
```javascript
const { loadAgentConfig } = require('./.aexos-core/core');
...
```javascript
const { loadConfigSections } = require('./.aexos-core/core');
...
```javascript
const { ContextDetector } = require('./.aexos-core/core');
...
```javascript
const { SessionContextLoader } = require('./.aexos-core/core');
...
```javascript
const { ElicitationEngine } = require('./.aexos-core/core');
...
```javascript
const { ElicitationSessionManager } = require('./.aexos-core/core');
...
```javascript
const { YAMLValidator, validateYAML } = require('./.aexos-core/core');
...
```javascript
const { PersonalizedOutputFormatter } = require('./.aexos-core/core');
...
*AEXOS (Cyryx) Core Module v2.0.0*
