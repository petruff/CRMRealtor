 ...
 *   - .aexos-core/project-config.yaml (L2) — team-shared project settings
 *   - ~/.aexos/user-config.yaml (L5) — cross-project user preferences
 *
 * Safety:
...
/**
 * Fields that belong in L5 User config (~/.aexos/user-config.yaml).
...
/**
 * Fields that belong in L2 Project config (.aexos-core/project-config.yaml).
...
function isLegacyMode(projectRoot) {
  const legacyPath = path.join(projectRoot, '.aexos-core', 'core-config.yaml');
  const frameworkPath = path.join(projectRoot, '.aexos-core', 'framework-config.yaml');
...
function createBackup(projectRoot) {
  const legacyPath = path.join(projectRoot, '.aexos-core', 'core-config.yaml');
  const backupPath = path.join(projectRoot, '.aexos-core', 'core-config.backup.yaml');
...
/**
 * Ensure the ~/.aexos/ directory exists.
 *
 * @returns {string} Path to ~/.aexos/ directory
 */
function ensureUserConfigDir() {
  const dir = path.join(os.homedir(), '.aexos');
...
/**
 * Write user config to ~/.aexos/user-config.yaml.
...
function writeUserConfig(userFields) {
  ensureUserConfigDir();
  const userConfigPath = path.join(os.homedir(), '.aexos', 'user-config.yaml');
...
/**
 * Write project config to .aexos-core/project-config.yaml.
...
function writeProjectConfig(projectRoot, projectFields) {
  const projectConfigPath = path.join(projectRoot, '.aexos-core', 'project-config.yaml');
