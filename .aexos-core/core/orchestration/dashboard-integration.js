 ...
 *
 * Features:
 * - AC1: Updates `.aexos/dashboard/status.json`
...
class DashboardIntegration extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    // Paths
    this.dashboardDir = path.join(this.projectRoot, '.aexos', 'dashboard');
    this.statusPath = path.join(this.dashboardDir, 'status.json');
    this.logsDir = path.join(this.projectRoot, '.aexos', 'logs');
    ...
    // In-flight background writes (see _track/drain).
    // Deliberate divergence from the original design, which fired status writes without
    // tracking them, so they can outlive the call that triggered them.
   ...
   *
   * Waits for in-flight status writes to settle before resolving, so callers
   * can safely tear down the project directory afterwards.
   *
  ...
  /**
   * Register an in-flight background side effect so it can be drained later.
   *
   * Node's EventEmitter invokes listeners fire-and-forget: an `async` listener
   * returns a promise nobody awaits, so its file writes can land after the
   * orchestrator call that triggered them has already returned. Tracking the
   * promise here lets `drain()` await it.
   *
   * The returned promise never rejects: these side effects are non-critical and
   * were previously swallowed entirely, so surfacing an I/O failure must not
   * break the orchestration flow (nor raise `unhandledRejection`).
   *
   * @private
   * @param {Promise<*>|*} work - Pending work to track
   * @returns {Promise<void>} Promise that settles when the work completes
   */
  _track(work) {
    const tracked = Promise.resolve(work)
      .catch((error) => {
        this._emitSafeError({ type: 'backgroundUpdate', error });
      })
      .then(() => {
   ...
   *
   * Never rejects (see `_track`). Safe to call when nothing is pending.
   *
   * @returns {Promise<void>}
   */
  async drain() {
    while (this._pending.size > 0) {
      const inFlight = Array.from(this._pending);
  ...
  _bindOrchestratorEvents() {
    ...
    // Register an async listener whose promise is tracked for draining.
    // Without this, every listener below is fire-and-forget (see _track).
    const on = (event, listener) => orch.on(event, (data) => this._track(listener(data)));

    // State changes
    on('stateChange', async (data) => {
      await this.updateStatus();
