...
class HierarchicalContextManager extends EventEmitter {
  ...
  _extractSourceMessages(messages) {
    ...
    for (const message of messages) {
      const nestedSources = message.metadata
        && message.metadata.aexos
        && Array.isArray(message.metadata.aexos.sourceMessages)
        ? message.metadata.aexos.sourceMessages
        : null;
  ...
  _truncateSummaryMessage(message) {
    ...
    cloned.metadata = {
      ...(cloned.metadata || {}),
      aexos: {
        ...(cloned.metadata && cloned.metadata.aexos ? cloned.metadata.aexos : {}),
