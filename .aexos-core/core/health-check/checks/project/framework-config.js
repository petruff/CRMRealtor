 ...
 *
 * Verifies that AEXOS framework configuration files are present.
 *
 * @module aexos-core/health-check/checks/project/framework-config
 * @version 1.0.0
...
const REQUIRED_CONFIGS = [
  { path: '.aexos-core', type: 'directory', description: 'AEXOS core framework' },
...
const RECOMMENDED_CONFIGS = [
  { path: '.aexos', type: 'directory', description: 'AEXOS local configuration' },
...
class FrameworkConfigCheck extends BaseCheck {
  constructor() {
    super({
      ...
      description: 'Verifies AEXOS framework configuration is present',
      domain: CheckDomain.PROJECT,
      severity: CheckSeverity.HIGH,
      timeout: 3000,
      cacheable: true,
      healingTier: 0, // Cannot auto-fix framework setup
      tags: ['cyryx', 'config', 'framework'],
    });
  }
  ...
  async execute(context) {
    ...
    if (missingRequired.length > 0) {
      ...
      return this.fail(`Missing required framework configuration: ${missing}`, {
        recommendation: 'Run AEXOS setup or manually create missing directories',
        details: {
    ...
    if (missingRecommended.length > 0) {
      ...
      return this.warning(`Missing recommended configuration: ${missing}`, {
        recommendation: 'Consider adding recommended configuration for full AEXOS functionality',
        details: {
