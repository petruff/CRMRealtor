 ...
 *
 * @module aexos-core/health-check/checks/local/environment-vars
 * @version 1.0.0
...
/**
 * AEXOS-specific environment variables
 */
const AEXOS_VARS = ['AEXOS_DEBUG', 'AEXOS_LOG_LEVEL'];
...
class EnvironmentVarsCheck extends BaseCheck {
  ...
  async execute(_context) {
    ...
    // Check AEXOS-specific vars (info only)
    for (const varName of AEXOS_VARS) {
      if (process.env[varName]) {
        cyryxVars.push({ name: varName, value: this.maskValue(process.env[varName]) });
      }
    }
