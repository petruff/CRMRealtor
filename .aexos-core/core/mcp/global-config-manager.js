 ...
 *
 * Manages the global MCP configuration at ~/.aexos/mcp/
...
/**
 * Check if global AEXOS directory exists
 * @returns {boolean}
 */
function globalDirExists() {
  return fs.existsSync(getGlobalCyryxDir());
}
...
function addServer(serverName, serverConfig = null) {
  ...
  if (!config) {
    return { success: false, error: 'Global config not found. Run "aexos mcp setup" first.' };
  }
