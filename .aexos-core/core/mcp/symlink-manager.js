 ...
 * @returns {string} Path to .aexos-core/tools/mcp
 */
function getProjectMcpPath(projectRoot = process.cwd()) {
  return path.join(projectRoot, '.aexos-core', 'tools', 'mcp');
}
...
function createLink(projectRoot = process.cwd(), options = {}) {
  ...
  if (!fs.existsSync(globalPath)) {
    return {
      success: false,
      error: 'Global MCP directory does not exist. Run "aexos mcp setup" first.',
      linkPath,
      globalPath,
    };
  }
