...
function handleHelp() {
  const usage = `
Usage: cyryx graph [command] [options]
...
Examples:
  cyryx graph --deps                        Show dependency tree
  cyryx graph --deps --format=json          Output as JSON
  cyryx graph --deps --format=html          Interactive HTML graph (opens browser)
  cyryx graph --deps --watch                Live DOT file for VS Code preview
  cyryx graph --deps --watch --format=html  Live HTML with auto-refresh
  cyryx graph --deps --watch --format=mermaid  Live Mermaid file
  cyryx graph --deps --watch --interval=10  Refresh every 10 seconds
  cyryx graph --stats                       Show entity stats and cache metrics
`.trim();
