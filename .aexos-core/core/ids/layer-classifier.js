 ...
 *
 * Layer Model:
 *   L1 (Framework Core)      — .aexos-core/core/, bin/, constitution.md
 *   L2 (Framework Templates)  — .aexos-core/development/, infrastructure/, product/
 *   L3 (Project Config)       — .aexos-core/data/, MEMORY.md, .claude/, *-config.yaml
...
const LAYER_RULES = [
  // --- L1: Framework Core ---
  { layer: 'L1', test: (p) => p.startsWith('.aexos-core/core/') },
  { layer: 'L1', test: (p) => p.startsWith('bin/') },
  { layer: 'L1', test: (p) => p === '.aexos-core/constitution.md' },

  // --- L3: Project Config (before L2 to catch MEMORY.md inside agents/) ---
  { layer: 'L3', test: (p) => p.startsWith('.aexos-core/data/') },
  ...
  { layer: 'L2', test: (p) => p.startsWith('.aexos-core/development/') },
  { layer: 'L2', test: (p) => p.startsWith('.aexos-core/infrastructure/') },
  { layer: 'L2', test: (p) => p.startsWith('.aexos-core/product/') },
