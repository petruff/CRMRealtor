...
const LOCK_FILE = path.resolve(REPO_ROOT, '.aexos-core/data/.entity-registry.lock');
const BACKUP_DIR = path.resolve(REPO_ROOT, '.aexos-core/data/registry-backups');
const AUDIT_LOG_PATH = path.resolve(REPO_ROOT, '.aexos-core/data/registry-update-log.jsonl');
const POPULATOR_SCRIPT = path.resolve(__dirname, '../../development/scripts/populate-entity-registry.js');
...
class RegistryUpdater {
  ...
  /**
   * Re-run the full populator so the file left on disk is byte-equivalent to
   * what `scripts/validate-registry-determinism.js` regenerates.
   *
   * INTENTIONAL DIVERGENCE FROM THE ORIGINAL DESIGN (owner-authorized). The
   * incremental path structurally cannot reproduce the populator's output:
   * `_handleFileCreate`/`_handleFileModify` call only `detectDependencies()`,
   * dropping the populator's `extractYamlDependencies()` (agents/workflows) and
   * `extractMarkdownCrossReferences()` (tasks/checklists/templates), which
   * collapses dependency lists and cascades into wrong `usedBy`/`lifecycle`;
   * `_detectCategory()` matches on `basePath` prefix and ignores each config's
   * glob, registering entities the populator never scans (e.g. `.yaml` under
   * `development/tasks`, whose glob admits Markdown only); deps/lifecycle are
   * reclassified only for touched entities; `metadata.resolutionRate` is never
   * recomputed; and `_applyCodeIntelEnrichments()` layers on data that depends
   * on external services. Since the determinism check is a required CI gate,
   * any hook-written registry that drifts blocks every PR.
   *
   * Invoked exactly as the gate invokes it, so both sides are produced by the
   * same command. Guarded to the canonical registry because the populator only
   * ever writes `REGISTRY_PATH` — instances pointed at another registry (tests,
   * custom roots) must keep the purely incremental path.
   */
  _reconcileWithPopulator() {
    if (path.resolve(this._registryPath) !== path.resolve(REGISTRY_PATH)) return;

    try {
      execFileSync(process.execPath, [POPULATOR_SCRIPT], { cwd: REPO_ROOT, stdio: 'pipe' });
    } catch (err) {
      // Never block the calling hook — leave the incremental write in place.
      console.warn(`[IDS-Updater] Registry reconcile skipped: ${err.message}`);
    }
  }
