...
function peerHasCore(root) {
  try {
    return fs.existsSync(path.join(root, '.aexos-core'));
  } catch {
    return false;
  }
}
...
async function run(context = {}) {
  ...
  const hub = resolvePeer(
    projectRoot,
    ['hub-framework', 'aexos-hub', 'framework-hub'],
    process.env.AEXOS_HUB_ROOT,
  );
  // Local lab folder name allowed via env only when set; avoid product tokens in source.
  const ent = resolvePeer(
    projectRoot,
    ['enterprise-framework', 'aexos-enterprise', 'AEXOS-enterprise'],
    process.env.AEXOS_ENTERPRISE_ROOT,
  );
