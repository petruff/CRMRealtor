...
describe('path-guard (CORE-SU.A3)', () => {
  ...
  describe('checkDenyList()', () => {
    it('blocks constitution and .git', () => {
      const constitution = path.join(PROJECT_ROOT, '.aexos-core/constitution.md');
  ...
  describe('validateWrite()', () => {
    ...
    it('denies deny-listed paths without traversal', () => {
      const result = validateWrite('.aexos-core/constitution.md', PROJECT_ROOT);
      expect(result.allowed).toBe(false);
  ...
  describe('getDenyList()', () => {
    it('does not include workspace/ product paths', () => {
      ...
      expect(WRITE_DENY_LIST).toEqual(expect.arrayContaining(['.git', '.aexos-core/constitution.md']));
    });
  });
});
